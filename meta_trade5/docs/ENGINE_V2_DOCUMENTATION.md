...
│    - Soma ponderada: vote_sum = S(signal_dir * weight)          │
...
config = {
    ...
    'ensemble_threshold': 0.0,  # 0 = modo SELECTOR
    'ensemble_min_votes': 1,    # 1 = qualquer modo dispara
    'risk_manager': {
        'daily_stop': -5000,
...
```python
# Se 1 modo quer entrar -> entra com esse modo
# Se multiplos querem -> escolhe o de MAIOR PESO (weight)
# Threshold = 0 e min_votes = 1 ativam o modo SELECTOR
...
```python
config = {
    'ensemble_threshold': 0.0,  # 0 = modo SELECTOR (qualquer sinal dispara)
...
| Versao | Mudancas |
|--------|----------|
| V139+ | 4 Layers (Base, Ensemble, Day Filter, Risk Manager), TP30, Grace Period, Slope Decay, Circuit Breaker, SELECTOR mode |
